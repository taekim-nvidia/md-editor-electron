  ...
  if (isElectron()) {
    try {
      const [statusRes, branchRes, logRes] = await Promise.all([
        ...
        window.electronAPI!.runGit(['log', '--oneline', '-10'], cwd).catch(() => ({ stdout: '', stderr: '', code: 0 })),
      ])
      return {
        ok: true,
...
export async function gitPull(cwd: string): Promise<ActionResult> {
  if (isElectron()) {
    try {
      ...
      return { ok: r.ok === true, output: r.stdout + r.stderr }
    } catch (err) {
...
export async function ghRepos(): Promise<{ ok: boolean; repos: GhRepo[]; error?: string }> {
  if (isElectron()) {
    try {
      ...
      if (r.ok !== true) return { ok: false, repos: [], error: r.stderr }
...
export async function ghClone(repo: string): Promise<CloneResult> {
  if (isElectron()) {
    ...
    try {
      ...
      return { ok: r.ok === true, path: destPath, output: r.stdout + r.stderr, action: 'pulled' }
    } catch {
...
export async function ghCommitPush(repoPath: string, message: string, addAll?: boolean): Promise<ActionResult> {
  if (isElectron()) {
    try {
      ...
      return {
        ok: commitR.ok === true && pushR.ok === true,
